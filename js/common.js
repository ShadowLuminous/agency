...
---------------------------------------------------*/
	...
	window.ScrollEffects = function() {
		...
		function calculatePixels() {
			...
			var bgColor = document.getElementById('webone-page-content').getAttribute('data-bgcolor');
		...
		if (isMobile()) {
			...
			function setViewPortHeight() {
				...
				gsap.set(".smooth-scroll main, .has-parallax, .webone-nav-wrapper, .webone-slider-wrapper:not(.content-slider), .showcase-lists .webone-sync-slider, .slider-fixed-content", { height: winHeight});
			}
		...
		$('#filters li a').on('click', function() {			
			var heroheight = $("#hero").height()/2 + $(".view-filters").height()*2;			
			if ($("body").hasClass("smooth-scroll")) {
				gsap.to(scrollbar, {duration: 0.7, scrollTop:heroheight, ease:Power4.easeInOut});
			} else {
				$("html,body").animate({scrollTop: heroheight}, 700);
			}
		});
		...
		if ($('body').hasClass("swap-logo")) {
			...
			ScrollTrigger.create({
				trigger: '.webone-header', // Set the trigger element
				start: 'top 120px',
				onEnter: function() {
				...
				onEnterBack: function() {
					...
					$('.webone-header').removeClass('swapped-logo');
				},
				onLeave: function() {
					...
					$('.webone-header').addClass('swapped-logo');			
				},
		...
		gsap.utils.toArray('.clipped-image-wrapper').forEach((clippedImageWrapper) => {
			...
			function setClippedImageWrapperProperties() {
				...
				gsap.set(clippedImage, { height: window.innerHeight - document.querySelector('.webone-header').offsetHeight * 2 });								
				gsap.set(clippedImageContent, { paddingTop: (window.innerHeight/2) + clippedImageContent.offsetHeight});
				gsap.set(clippedImageWrapper, { height: window.innerHeight + clippedImageContent.offsetHeight - document.querySelector('.webone-header').offsetHeight});
		...
		function setMainPageContentMargins() {
			var winHeight = $(window).height();
			var footerHeight = $('.webone-footer').height();
		...
		gsap.to('.page-nav-caption', {
			...
			scrollTrigger: {
				trigger: '#page-nav',
				start: startValue,
				end: () => `+=${$('#page-nav').outerHeight() + $('.webone-footer').outerHeight()}`,
				scrub: true,
			}			
		});
			...
			imagesLoaded('body', function() {
				setTimeout(function() {
					...
					changeHeaderColor.forEach(function(changeHeaderElement) {						
						var pageHeader = document.querySelector('.webone-header');							
		...
		if ($('#webone-page-content').hasClass("light-content")) {
			if ($('#project-nav').hasClass("change-header")) {
		  		imagesLoaded('body', function() {
					setTimeout(function() {	
						var pageNav = document.querySelector('#project-nav.change-header');
						if (pageNav) {
							var pageContent = document.querySelector('#webone-page-content');	
		...
		if ($('#webone-page-content').hasClass("dark-content")) {
			if ($('#project-nav').hasClass("change-header")) {
		  		imagesLoaded('body', function() {
					setTimeout(function() {	
						var pageNav = document.querySelector('#project-nav.change-header');
		...
		gsap.set('.header-gradient', {
			delay: 0.5,
			backgroundColor: document.getElementById('webone-page-content').getAttribute('data-bgcolor'),			
			onUpdate: function() {
			var bgColor = document.getElementById('webone-page-content').getAttribute('data-bgcolor');
		...
		disableHeaderGradient.forEach(function(noHeaderGradient) {						
			...
			gsap.to(noHeaderGradient, {
				scrollTrigger: {
					...
					onEnter: function() {
						gsap.to('.header-gradient', { duration: 0.1, opacity: 0, ease: Power2.easeInOut });
					},
					onEnterBack: function() {
						gsap.to('.header-gradient', { duration: 0.1, opacity: 0, ease: Power2.easeInOut });
					},
					onLeave: function() {
						gsap.to('.header-gradient', { duration: 0.1, opacity: 1, ease: Power2.easeInOut });
					},
					onLeaveBack: function() {
						gsap.to('.header-gradient', { duration: 0.1, opacity: 1, ease: Power2.easeInOut });
					}
		...
		$('a.ajax-link').on('click', function() {
			...
			gsap.to('#webone-logo img.white-logo, #webone-logo img.black-logo', { duration: 0.5, delay:0, opacity: "",  ease: Linear.easeNone, });
			gsap.to('.classic-menu .flexnav li a', { duration: 0.5, delay:0, color: "", ease: Linear.easeNone, });			
		...
		$("#hero #hero-caption.landing-page-title .inner").mouseenter(function(e) {    
			// Animații de început
			gsap.to('#ball', {
				duration: 0.3,
				borderWidth: '2px',
			...
			// Adăugăm prima imagine imediat la prima intrare
			$("#ball").append('<img class="eyes-icon" src="images/eyes.png" style="transform:scale(0.75);">');
		
			// Funcție pentru schimbarea imaginilor între cele trei
			let imageIndex = 0;
			const images = [
				'<img class="eyes-icon" src="images/eyes1.png" style="transform:scale(0.75);">',
				'<img class="eyes-icon" src="images/eyes.png" style="transform:scale(0.75);">'  // A treia imagine
			];
		
			function switchImage() {
				const ball = $("#ball");
				ball.find('img').remove(); // Eliminăm imaginea curentă
		
				// Adăugăm următoarea imagine din array
				ball.append(images[imageIndex]);
		
				// Incrementăm indexul și resetăm la începutul array-ului dacă e cazul
				imageIndex = (imageIndex + 1) % images.length;
			}
		
			// Setăm intervalul pentru schimbarea imaginii la fiecare 3 secunde
			const imageInterval = setInterval(switchImage, 1000);
		
			// Oprim schimbarea icoanelor când mouse-ul părăsește zona
			$("#hero #hero-caption .inner").mouseleave(function() {
				...
				$("#ball").find('img').remove();
				 clearInterval(imageInterval); // Oprim intervalul
			});
		});
...
---------------------------------------------------*/	
	...
	window.FirstLoad = function() {
		...
		if ($("#webone-page-content").hasClass("light-content")) {
			$(".webone-nav-wrapper").css('background-color', function () {
				return $(".webone-header").data('menucolor')
			});
			
			gsap.to("main", {duration: 0.5, backgroundColor: document.querySelector("#webone-page-content").getAttribute("data-bgcolor"), ease:Power2.easeInOut});
			
			$('#magic-cursor').addClass('light-content');
			if( $('#hero').length > 0 ){						
				if( $('#hero').hasClass("has-image")) {	
					$(".webone-header").css('background-color', 'transparent');
				} else {
					if ($(".webone-header").hasClass("fullscreen-menu")) {
						$(".webone-header").css('background-color', 'transparent');
					} else {
						if( $('#blog').length > 0 ){
							$(".webone-header").css('background-color', '#171717');
						}
						if( $('#post').length > 0 ){
							$(".webone-header").css('background-color', '#171717');
						}
			...
			if( $('#hero').length > 0 ){	
				if( $('#hero').hasClass("has-image")) {	
					...
					if ($(".webone-header").hasClass("fullscreen-menu")) {
						...
						if( $('#blog').length > 0 ){
							$(".webone-header").css('background-color', '#fff');
						}
						if( $('#post').length > 0 ){
							$(".webone-header").css('background-color', '#fff');
						}
		...
		$('a.ajax-link').on('click', function() {
			...
			gsap.to("nav", {duration: 0.3, backgroundColor: document.querySelector("#webone-page-content").getAttribute("data-bgcolor")});
		...
		$('.webone-nav-wrapper .ajax-link').on('click', function() {
		...
		$('#burger-wrapper, .menu .button-text').on('click', function() {
			$('#menu-burger, .webone-nav-wrapper').toggleClass('open');			
			setTimeout( function(){			
				if ($('#menu-burger').hasClass("open")) {
					
					gsap.to('.webone-nav-wrapper', {duration: 0.3, opacity:1, ease:Power2.easeInOut});
					
					$('.webone-header').addClass('over-sidebar').addClass('over-white-section');
					...
					gsap.set($(".webone-nav-wrapper ul ul li"), {y: 0, opacity:1});
					...
					setTimeout( function(){
						...
						$('.webone-header').removeClass('over-sidebar')
						setTimeout( function(){
							$('.webone-header').removeClass('over-white-section');
						} , 350 );
					} , 500 );
				}							
			} , 20 );
				  ...
				  $body.on('mouseup mousemove', function handler(evt) {
					if (evt.type === 'mouseup') {
					  ...
					  $('#itemsWrapperLinks .trigger-item-link, #itemsWrapperLinks .trigger-item-link-secondary').on('click', function() {					
							...
							/*if (!$('#webone-page-content').hasClass("light-content")) {
								...
								if (!$('.portfolio').hasClass("portfolio-shortcode")) {
									if (!parent_item.hasClass("change-header")) {
										$('#webone-page-content').delay(700).queue(function(next){							
							...
							$('.webone-slider-wrapper .trigger-item').each(function(){
							...
							gsap.to('#hero, #show-filters, .item-caption-wrapper, .showcase-portfolio .slide-caption, #page-nav, .webone-footer, .fadeout-element', {duration: 0.5, opacity:0, ease:Power4.easeInOut});
...
---------------------------------------------------*/
	...
	window.Sliders = function() { 
		...
		if( $('.content-slider').length > 0 ){
			...
			slider = new weboneSlider('.content-slider', { 
			...
			$(".slider-button-prev").mouseenter(function(e) {				
				...
				if (modifyMouseColor) {
					...
					if ($(this).parents(".webone-slider-wrapper").hasClass("light-cursor")) {
						$("body").addClass("drag-cursor-white");
						gsap.to('#ball', {duration: 0.2, borderWidth: '2px', scale: 1, borderColor:'#fff', backgroundColor:'#fff'});
					} else if ($(this).parents(".webone-slider-wrapper").hasClass("dark-cursor")) {
			...
			$('.content-slider.looped-carousel .webone-slider, .content-slider.small-looped-carousel .webone-slider').on('mouseenter mousemove', function() {	
...
---------------------------------------------------*/
	...
	window.Lightbox = function() {
		...
		function showDetails(item) {
			...
			$('body').prepend(`<div class="webone-img-popup">
				<div class="webone-img-popup-bg-close"></div>				
				<div class="webone-img-popup-viewport">
					<div class="webone-img-popup-preloader"><div></div><div></div><div></div><div></div></div>					
					<img />
					<div class="webone-img-popup-close link"></div>						
				</div>				
				<div class="webone-img-popup-prev link"></div>
				<div class="webone-img-popup-next link"></div></div>`);
			
			const details = document.querySelector('.webone-img-popup');
			const detailsBgClose = document.querySelector('.webone-img-popup-bg-close');
			const detailsClose = document.querySelector('.webone-img-popup-close');
			const detailsPreloader = document.querySelector('.webone-img-popup-preloader');
			const detailImage = document.querySelector('.webone-img-popup img');
			const detailPrev = document.querySelector('.webone-img-popup-prev');
			const detailNext = document.querySelector('.webone-img-popup-next');
		...
		function showVideoDetails(event, item) {
			...
			$('body').prepend(`<div class="webone-video-popup">
				<div class="webone-video-popup-bg-close"></div>				
				<div class="webone-video-popup-viewport">
					<div class="webone-video-popup-preloader"><div></div><div></div><div></div><div></div></div>					
					<iframe class="webone-video-popup-iframe" frameborder="0" allow="autoplay"></iframe>
					<div class="webone-video-popup-close link"></div>						
				</div>				
				<div class="webone-img-video-prev link"></div>
				<div class="webone-img-video-next link"></div></div>`);
			
			const details = document.querySelector('.webone-video-popup');
			const detailsBgClose = document.querySelector('.webone-video-popup-bg-close');
			const detailsClose = document.querySelector('.webone-video-popup-close');
			const detailsPreloader = document.querySelector('.webone-video-popup-preloader');
			const detailIframe = document.querySelector('.webone-video-popup iframe');
...
---------------------------------------------------*/
	...
	window.Core = function() {
		...
		if (!isMobile() && !$('body').hasClass("disable-cursor")) {
			...
			$(".webone-button .sticky.left").mouseenter(function(e) {		  
				var rcBounds = $(this)[0].getBoundingClientRect();		  
			...
			$(".webone-button .sticky.right").mouseenter(function(e) {		  
				var rcBounds = $(this)[0].getBoundingClientRect();		  
			...
			$(".webone-button .parallax-wrap.icon-wrap").mouseenter(function(e) {
			...
			$(".webone-button .sticky").mouseenter(function(e) {
				if ($('#webone-page-content').hasClass("light-content")) {
			...
			$(".webone-button .parallax-wrap").mouseenter(function(e) {
				if ($('#webone-page-content').hasClass("light-content")) {
			...
			//Blog Hover Effects			
			$("#blog-page-nav .page-numbers li a, .post-page-numbers, #post-content a, #post-form a, #post-comments a, .wp-block-search__button, .webone-sidebar-widget a").mouseenter(function(e) {	
				gsap.to('#ball', {duration: 0.2, borderWidth: '1px', scale: 1, opacity:0});
			});	
					
			$("#blog-page-nav .page-numbers li a, .post-page-numbers, #post-content a, #post-form a, #post-comments a, .wp-block-search__button, .webone-sidebar-widget a").mouseleave(function(e) {
				gsap.to('#ball', {duration: 0.3, borderWidth: '4px', scale:0.5, opacity:1});
			});
		}
			  ...
			  $(window).on('popstate', function() {
				...
				section.load(url+' .cd-main-content > *', function(event){
				  // load new content and replace <main> content with the new one
				 	...
				 	var webone_title = event.match(/<title[^>]*>([^<]+)<\/title>/)[1];
					$('head title').html( webone_title );
