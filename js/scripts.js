...
---------------------------------------------------*/			
	...
	function InitShuffleElements() {
		...
		shuffleFunctions.applyShuffleEffect(".showcase-portfolio .webone-item .slide-inner", {
			velocity: 30,
			shuffleIterations: 8,
			childSelector: '.slide-title span, .slide-cat span'
		});
		
		shuffleFunctions.applyShuffleEffect(".webone-slide .slide-inner-height", {
			velocity: 30,
			shuffleIterations: 8,
			childSelector: '.slide-title span, .slide-cat span'
		});
...
---------------------------------------------------*/	
	...
	function CleanupBeforeAjax(){		
		...
		weboneSlider.instances.forEach(slider => slider.off());
		weboneSlider.instances = [];
	}
...
---------------------------------------------------*/
	...
	function PageLoad() {		
		...
		gsap.set($(".webone-header"), {yPercent:-50, opacity:0});
		...
		function initOnFirstLoad() {
			...
			imagesLoaded('body', function() {
				...
				gsap.to($(".webone-header"), { duration: 0.45, opacity:1, yPercent:0, delay:0.25, ease: Power2.easeOut, });
	...
	function PageLoadActions() {
		...
		if (!isMobile()) {
			...
			$("#page-nav .next-ajax-link-page").on('mouseenter', function() {
				var modifyMouseColor = $('#webone-page-content').attr("data-modify-color");
				if (modifyMouseColor) {
		...
		if (!$("body").hasClass("disable-ajaxload")) {
			...
			$('#page-nav .next-ajax-link-page').on('click', function() {	
				...
				$('.webone-header').removeClass('white-header');
				$("#app").remove();
				...
				gsap.to($("#main-page-content, #hero, .webone-footer, .webone-footer-nav"), {duration: 0.3, opacity:0, ease:Power4.easeOut});
				
				if ($("#page-nav").hasClass("move-nav-onload")) {
					$("body").addClass("load-next-page");
					if ($("body").hasClass("smooth-scroll")) {
						var moveNav = $("#content-scroll").height() - ( $("#hero").height() / 2 )  - ( $("#page-nav").height()  ) / 2 - $(".webone-footer").height() / 2	
					} else {
						var moveNav = window.innerHeight - ( $("#hero").height() / 2 )  - ( $("#page-nav").height()  ) / 2 - $(".webone-footer").height() / 2	   
					}
			...
			$('#page-nav .next-ajax-link-page').on('click', function() {					
				...
				gsap.to($(".webone-footer, .webone-footer-nav"), {duration: 0.3, opacity:0, delay:0, ease:Power2.easeInOut});
			});
		...
		if (!$("body").hasClass("disable-ajaxload")) {
			...
			$('.next-ajax-link-project').on('click', function() {
				...
				if ($(".webone-header").hasClass("swapped-logo")) {
					...
					gsap.to($("#webone-logo"), {duration: 0.2, opacity:0, onComplete: function() {
						imgLogoWhite.src = originalSrcWhite;
						imgLogoBlack.src = originalSrcBlack;
						gsap.to($("#webone-logo"), {duration: 0.2, opacity:1});
					}});
				...
				gsap.to($(".webone-footer, .all-works"), {duration: 0.3, opacity:0, ease:Power2.easeInOut});
		...
		if( $('#project-nav').length > 0 ){		
			...
			const pageContent = document.getElementById("webone-page-content");
			...
			ScrollTrigger.create({
				...
				onLeaveBack: function() {
					...
					gsap.set("#webone-logo img, .classic-menu .flexnav li a, .button-wrap.menu, .button-icon-link", { clearProps: "all" });
				},			
				scrub: true,
			});	
			...
			if ($("#project-nav").hasClass("change-header")) {
				...
				if (changeMainColor) {
					...
					imagesLoaded('body', function() {
						...
						const blackLogoOpacity = gsap.getProperty('#webone-logo img.black-logo', 'opacity');
						const whiteLogoOpacity = gsap.getProperty('#webone-logo img.white-logo', 'opacity');
						...
						ScrollTrigger.create({
							...
							onEnter: function( st ) {
								...
								gsap.fromTo('#webone-logo img.black-logo', {
								...
								gsap.fromTo('#webone-logo img.white-logo', {
									opacity: whiteLogoOpacity
								}, {
									duration: 1, 
									opacity: blackLogoOpacity,
...
---------------------------------------------------*/	
	...
	function ShowcasePortfolio() {
		...
		gsap.utils.toArray('.webone-item .pixels-cover').forEach((pixelWrapper, index) => {
			const pixelAnimation = pixelWrapper.querySelectorAll(".pixel");
		...
		if( $('.showcase-portfolio').length > 0 ){
			...
			function filter() {				
				...
				var state = Flip.getState('.webone-item');
				var projects = document.querySelectorAll('.webone-item');
				var startHeight = gsap.getProperty(".showcase-portfolio", "height");
				
				var filters = document.querySelectorAll('.filter-option.is_active');
				var parallaxItems = document.querySelectorAll('.showcase-portfolio .webone-item.vertical-parallax .slide-inner');
			...
			function resetItems() {
				$('.expand-grid .webone-item').removeClass('expanded not-expanded').addClass('default');
			}
			...
			function expandItem($item) {
				...
				$('.expand-grid .webone-item').slice(start, end).not($item).addClass('not-expanded').removeClass('default');
			}		
		
			expandItem($('.expand-grid .webone-item').eq(1));
		
			$('.expand-grid .webone-item').on('mouseenter', function() {
			...
			if( $('.filp-grid').length > 0 ) {
				...
				if (!isMobile()) {
					...
					const weboneItems = document.querySelectorAll('.showcase-portfolio .webone-item');
					
					
					weboneItems.forEach(item => {
					...
					if (showcasePortfolio.classList.contains('filp-grid')) {
						...
						const weboneItems = showcasePortfolio.querySelectorAll('.webone-item');						
						weboneItems.forEach(item => {
					...
					const overlappingThumbs = document.querySelectorAll('.showcase-portfolio .webone-item .img-mask');
			...
			if (!isMobile()) {	
				...
				$(".showcase-portfolio .webone-item .slide-inner").on('mouseenter', function() {
					$('#ball p').remove();
					var $this = $(this);			
...
---------------------------------------------------*/
	...
	function ShowcaseCarousel() {
		...
		if( $('.showcase-carousel').length > 0 ){
			...
			gsap.set($(".showcase-carousel .webone-slider .slide-inner-height"), { scale: 0.8 });
				...
				setTimeout(function() {	
					...
					function animatePixels(selector, minDelay, maxDelay) {
						gsap.utils.toArray(selector).forEach((pixelWrapper) => {
							const pixelAnimation = pixelWrapper.querySelectorAll(".webone-slider .pixel");
					...
					animatePixels('.webone-slide-active .pixels-cover', 0, 0.3);
					animatePixels('.webone-slide:not(.webone-slide-active) .pixels-cover', 0.3, 0.6);
					
					gsap.to($(".showcase-carousel .webone-slider .slide-inner-height"), { duration: 0.6, scale: 1, ease: Power2.easeOut });
			...
			slider = new weboneSlider('.showcase-carousel', { 
				direction: 'horizontal',
				ease: 0.065, 
				snap:false,
				outer: '.webone-slider',
				inner: '.webone-slider-viewport',
				navigation: {
				...
				on: {	
					...
					slideLeaveViewport : function( slide ) {
						gsap.set($('.webone-slider div:not(.webone-slide-visible) .slide-effects'), { y: "" });		
					},
				},
			});
			...
			if (!isMobile()) {
				...
				$('.webone-slider').on('mousedown', function (evt) {
					$('.webone-slider').on('mouseup mousemove', function handler(evt) {
						if (evt.type === 'mouseup') {					  
							// click
							gsap.to('#ball', {duration: 0.2, borderWidth: '4px', scale:0.5, borderColor:'#999999', backgroundColor:'transparent'});
						...
						$('.webone-slider').off('mouseup mousemove', handler);
					});
				});
				
					
				$('.webone-slider').on('mouseup touchend', function() {
				...
				$(".showcase-carousel .webone-slide .slide-inner-height").on('mouseenter', function() {	
					if (!$('body').hasClass('scale-drag-x')) {
						...
						$('.webone-slide').addClass('disable');						
						$(this).closest(".webone-slide").removeClass("disable");
					}			
				}).on('mouseleave', function() {					
					if (!$('body').hasClass('scale-drag-x')) {
						gsap.to('#ball', {duration: 0.2, borderWidth: '4px', scale:0.5, borderColor:'#999999', backgroundColor:'transparent'});
						...
						$(this).closest(".webone-slide").addClass("disable");
						$('.webone-slide').removeClass('disable');	
					}
				});
...
---------------------------------------------------*/
	...
	function ShowcaseGallery() {
		...
		if( $('.showcase-gallery').length > 0 ){
			...
			if (isMobile()) {
				$(".webone-slider-wrapper").removeClass("preview-mode-enabled");
			}
			
			var slideElements = $(".webone-slide");
            ...
            $('#webone-page-content').append(bgPixels);
			...
			gsap.set($(".showcase-gallery .webone-slider .slide-inner-height"), { opacity: 0 });
			...
			if (!$('body').hasClass("show-loader")) {
				...
				setTimeout(function() {
					gsap.to($(".showcase-gallery .webone-slider .webone-slide .slide-inner-height"), { duration: 0.5, opacity: 1, delay: 0.2, ease: Power2.easeInOut });
					var gallerySlideClasses = [".webone-slide-prev-two", ".webone-slide-prev", ".webone-slide-active", ".webone-slide-next", ".webone-slide-next-two"];
					gallerySlideClasses.forEach(function(gallerySlideClass, index) {
						var gallerySlide = $(".showcase-gallery .webone-slider " + gallerySlideClass + " .slide-inner-height");
						var delay = 0 + index * 0.1;
						gsap.set(gallerySlide, { yPercent: 250 });
						gsap.to(gallerySlide, { duration: 1.5, yPercent: 0, delay: delay, ease: Power2.easeOut, onComplete: function() {
			...
			slider = new weboneSlider('.webone-slider-wrapper', { 
				direction: 'vertical',
				ease: 0.065, 
				outer: '.webone-slider',
				inner: '.webone-slider-viewport',
					...
					init : function(slide) {
						...
						if ($('body').hasClass("show-loader")) {
							...
							imagesLoaded('body', function() {
								...
								gsap.to($(".showcase-gallery .webone-slider .webone-slide .slide-inner-height"), { duration: 0.5, opacity: 1, delay: 0.4, ease: Power2.easeInOut });
			...
			const showGrid = () => {
				...
				document.getElementById("webone-page-content").classList.add('no-css-animation');
				...
				gsap.set(currentImage.closest(".webone-slide"), {zIndex: 0});
				...
				if (document.getElementById('webone-page-content').classList.contains('dark-content')) {
					if (currentImage.classList.contains('change-header')) {
						gsap.to('#webone-logo img.black-logo', { duration: 0.5, delay:0.15, opacity: 0, ease:Power2.easeInOut });	
						gsap.to('#webone-logo img.white-logo', { duration: 0.5, delay:0.15, opacity: 1, ease:Power2.easeInOut });						
				...
				} else if (document.getElementById('webone-page-content').classList.contains('light-content')) {
					if (currentImage.classList.contains('change-header')) {
						gsap.to('#webone-logo img.black-logo', { duration: 0.5, delay:0.15, opacity: 1, ease:Power2.easeInOut });	
						gsap.to('#webone-logo img.white-logo', { duration: 0.5, delay:0.15, opacity: 0, ease:Power2.easeInOut });						
				...
				gsap.to(".webone-slider .webone-slide .trigger-item", { duration: 1, opacity:0, scale:0.7, ease:Power2.easeOut });
			...
			const hideGrid = () => {
				...
				gsap.to(".webone-slider .webone-slide .trigger-item", { duration: 0.5, opacity:1, scale:1, delay:0.2, ease: 'power3.inOut' });
				...
				Flip.from(flipstate1, {
					...
					onComplete: function() {
						...
						const triggeredItem = document.querySelector('.webone-slide.triggered-item');
						if( triggeredItem != null ){							
							triggeredItem.classList.remove('triggered-item');
						}
						
						const weboneSlides = document.querySelectorAll('.webone-slide');
						weboneSlides.forEach(slide => {
						  	slide.style.zIndex = '';
						...
						document.getElementById("webone-page-content").classList.remove('no-css-animation');
						gsap.set($(".showcase-gallery .slide-caption"), {clearProps: "opacity"});
						
					}
				})								
				
				gsap.to('.header-gradient', { duration: 0.4, backgroundColor: $("#webone-page-content").data("bgcolor"), ease:Power2.easeInOut });
				gsap.to("main", { duration: 0.4, backgroundColor: $("#webone-page-content").data("bgcolor"), ease: Power2.easeInOut });
				gsap.to("#bg-pixels", { duration: 0.4, delay:0.15, opacity:0.75, ease: Power2.easeInOut });			
				
				
				if (document.getElementById('webone-page-content').classList.contains('dark-content')) {
					if (currentImage.classList.contains('change-header')) {
						gsap.to('#webone-logo img.black-logo', { duration: 0.4, opacity: 1, ease:Power2.easeInOut });	
						gsap.to('#webone-logo img.white-logo', { duration: 0.4, opacity: 0, ease:Power2.easeInOut });						
					...
					if (currentImage.classList.contains('change-header')) {						
						gsap.to('#webone-logo img.black-logo', { duration: 0.4, opacity: 0, ease:Power2.easeInOut });	
						gsap.to('#webone-logo img.white-logo', { duration: 0.4, opacity: 1, ease:Power2.easeInOut });						
				...
				gsap.to($("footer .link-text, .webone-pagination, #filters-wrapper"), {duration: 0.3, opacity:1, y:0, stagger:0.05, delay:0.4, ease:Power2.easeInOut});
				gsap.to('#ball', {duration: 0.2, borderWidth: '4px', scale:0.5, borderColor:'#999999', backgroundColor:'transparent'});
				gsap.to('#ball-loader', {duration: 0.2, borderWidth: '4px', top: 0, left: 0});
				$("#ball").removeClass("with-blur color-cursor");
				$('#ball p').remove();
			...
			const getDOMElements = () => {
				...
				const items = [];
				[...document.querySelectorAll('.webone-slide')].forEach(item => {
					items.push(new Item(item));
				});
						
				// Cloned items
				const itemsCloned = [];
				[...document.querySelectorAll('.webone-slide-clone')].forEach(item => {
				...
				return {					
					items: items,					
					grid: gridItems,					
					currentItem: new Item( document.querySelector('.webone-slide.triggered-item') ),
				...
				items.forEach((triggerItem) => {
					...
					triggerItem.addEventListener('click', (event) => {
						...
						if( $('.showcase-gallery').length > 0 ){
							...
							event.currentTarget.closest('.webone-slide').classList.add('triggered-item');
							showGrid();
						}
					});
			...
			if (!isMobile()) {
				...
				$(".showcase-gallery.preview-mode-enabled .webone-slide .slide-inner-height").on('mouseenter', function() {	
			...
			var currentSlide = null;
				...
				$('.webone-slide .slide-inner-height').on('mouseenter', function() {
					
					var slide = $(this).closest(".webone-slide").data('slide');
		...
		if ($(".projects-list-wrapper").hasClass("invert-header-color")) {
			...
			changeMainColor.forEach(function(changeHeaderColor) {
				...
				imagesLoaded('body', function() {
					...
					const bgPageContent = document.getElementById("webone-page-content");
					...
					const scrollTriggerEnter = {
						...
						onLeaveBack: function() {
							bgPageContent.classList.remove('no-css-animation');
							gsap.set('#webone-logo img.black-logo, #webone-logo img.white-logo', {opacity: "" });
						}
					};
					...
					$('.trigger-item').on('click', function() {
						...
						gsap.to('#webone-logo img.white-logo, #webone-logo img.black-logo', { duration: 0.5, delay:1, opacity: "",  ease: Linear.easeNone, });
